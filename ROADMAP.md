...
- 作用域 CRUD（API + UI）：建 / 改 / 禁即时生效，不重启进程
- **认证先行**：token + Host/Origin 校验（防 DNS rebinding）——
  无认证的可写接口不允许存在（安全红线第 6 条）
- `lessord --open`：启动即开默认浏览器指向本机控制台（评审提出的
  桌面端挑战方案，与桌面端并存，现场数据裁决）

**验收**：界面上新建作用域后下一个 DISCOVER 立即得到应答；未带 token 的
写请求被拒；非法配置被 core 校验挡下且不影响运行中的作用域。
