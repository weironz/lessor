...
已完成（v0.0.2 之后，随"桌面端直接进界面"一并落地）：

- [x] `POST /api/scopes` 建作用域 + 建完自动起监听器
- [x] 写操作守卫：Host 必须是 IP 字面量（防 DNS rebinding）、
      `--token` 时写操作必须带 Bearer（安全红线第 6 条）
- [x] `--serve-empty`：零作用域/零监听器启动，界面上开工

剩余：

- [ ] 作用域改 / 删 / 启停（`PATCH` / `DELETE /api/scopes/{id}`）——
      删除时要处理该网段已有的租约与监听器
- [ ] 静态保留的增删（现场把 BMC 钉死到规划地址就靠它）
- [ ] `lessord --open`：启动即开默认浏览器指向本机控制台
      （评审提出的桌面端挑战方案，与桌面端并存，由现场数据裁决）

**验收**：界面上新建作用域后下一个 DISCOVER 立即得到应答（已达成）；
改/删作用域即时生效且不影响其他作用域；未带 token 的写请求被拒（已达成）。
