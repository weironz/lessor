...
**前后端一体**：前端构建产物由 rust-embed 打进 `lessord`，运行时只有一个进程。
典型场景是"机房上架、插上网线、临时发个地址"，现场要的是一个文件拷过去就能跑，
不是一套编排。桌面端是同一份界面的外壳，`LESSOR_URL` 可以指向机房里另一台机器 ——
界面能远程看，但它本身不提供服务。

**构建顺序因此是固定的**：`ui/dist` 必须先存在，否则 `lessord` 编译不过。
先 `cd ui && bun install && bun run build`，再 `cargo build`。

完整的分层、技术选型与取舍见 [docs/architecture.md](docs/architecture.md)。
