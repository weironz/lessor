...
- **桌面端"关于"与自动更新**：点右上角版本号打开，里面能检查更新、
  确认后自动下载安装并重启。更新包带 minisign 签名，桌面端用内置公钥
  校验，验不过就拒装 —— 没有这道校验，谁能替换发布物谁就能在客户机器上
  执行代码。**确认更新会先停掉本机 lessord**（Windows 上文件被占用装不上去），
  但只停桌面端自己拉起的那个；界面连的是外部实例时不会去动它。
  首次启用需要仓库管理员生成密钥对，见 [docs/desktop-update.md](docs/desktop-update.md)
- 浏览器里打开同一个界面时，"关于"照常可看，只是没有检查更新 ——
  那里没有"程序"可更新

### Fixed

- **改了前端却不会触发 lessord 重新编译。** 界面是 rust-embed 在编译期打进
  二进制的，但 cargo 只盯 `.rs` 文件 —— `ui/dist` 变了而没有 Rust 代码变动时，
  它认为不用重编，二进制里还是上一次的界面。症状很误导：`cargo build` 明明
  跑了几十秒（时间花在依赖上），起来的还是旧界面。补了 `build.rs` 声明依赖。
  CI 每次干净构建所以没暴露，只有本地开发会中招
...
- 实时报文日志带上客户端自报的 option 60（设备类型）—— 现场一屏 MAC
  谁也认不出哪台是哪台。**未应答的事件也带**，那正是要排查的那一台
- 租约表里的地址可以点开，直接打开该设备的 Web 界面（`https://<地址>/`）——
  上架时拿到地址的下一步几乎总是"点进去看看"
